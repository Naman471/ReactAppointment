...
const AddAppointment = ({onSendAppointment,lastId}) => {
    ...
    function formDataPublish() {
      ...
      if(appointmentInfo.ownerName=='' || appointmentInfo.petName=='' || appointmentInfo.aptDate=='' || appointmentInfo.aptNotes=='')
      {
    ...
    return (
        <div className="pt-6 pd-2 px-5">
      <button onClick = { () => {setToggleForm(!toggleForm)}}
      className={`bg-blue-400 text-white px-4 py-3 w-full font-bold text-centre ${toggleForm ? 'rounded-t-md' : 'rounded-md' }`}>

      <div><BiCalendarPlus className="inline-block align-text-top" />  Add Appointment</div>
      </button>
      {
        toggleForm && 
        <div className="border-r-2 border-b-2 border-l-2 border-light-blue-500 rounded-b-md py-8 px-8">
        ...
        <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start  sm:pt-5">
          ...
          <div className="mt-1 sm:mt-0 sm:col-span-2">
            <input type="text" name="ownerName" id="ownerName" value={formData.ownerName}
              ...
              className="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-xs sm:text-sm border-2 border-gray-300 rounded-md" />
          </div>
        </div>
        ...
        <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start  sm:pt-5">
          ...
          <div className="mt-1 sm:mt-0 sm:col-span-2">
            <textarea id="aptNotes" name="aptNotes" rows="3" value={formData.aptNotes}
              ...
              className="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border-2 border-gray-300 rounded-md" placeholder="Detailed comments about the condition"></textarea>
          </div>
        </div>
